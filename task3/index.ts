...
import { IUnnormalizedItem, INormalizeData, INormalizeItem } from "./types";
...
const normalizeData = (unnormalizedData: IUnnormalizedItem[]) => {
  let normalizeData: INormalizeData = {
    allIds: [],
    byId:{}
  };
    if (unnormalizedData.length) {
        unnormalizedData.forEach((item: IUnnormalizedItem, index) => {
          normalizeData.allIds.push(item.id)
          normalizeData.byId[item.id] = {
            ...item
          }
        });
  }
